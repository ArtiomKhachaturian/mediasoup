...
class PoolAllocator : public BufferAllocator, private TimerHandle::Listener
{
    ...
    using StackChunks = std::array<std::shared_ptr<PoolMemoryChunk>, count>;
...
private:
    ...
    static std::shared_ptr<PoolMemoryChunk> GetAcquired(const TSerialChunks& chunks);
    static std::shared_ptr<PoolMemoryChunk> GetAcquired(size_t size, const HeapChunksMap& chunks);
    static std::shared_ptr<PoolMemoryChunk> GetAcquired(HeapChunksMap::const_iterator from,
                                                        HeapChunksMap::const_iterator to);
    static std::shared_ptr<HeapChunk> CreateHeapChunk(size_t alignedSize);
    std::shared_ptr<PoolMemoryChunk> GetStackChunk(size_t alignedSize) const;
    std::shared_ptr<PoolMemoryChunk> AcquireHeapChunk(size_t alignedSize);
